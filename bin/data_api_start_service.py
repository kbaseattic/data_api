...
def main():
    ...
    if args.service not in SERVICE_NAMES:
        g_log.info("Unrecognized service name {}".format(args.service))
        sys.exit(1)
    ...
    # get config
    g_log.info("Attempting to read config from " + args.config)
    if config.has_section(global_stanza_name):
        g_log.info("Reading global config from stanza " + global_stanza_name)
    ...
    if config.has_section(service_stanza_name):
        g_log.info("Reading " + args.service + " config from stanza " + service_stanza_name)
    ...
    if service_port is None:
        g_log.error("Service port not defined!")
        return 1

    if redis_host is not None and redis_port is not None:
        g_log.info("Activating REDIS at host:{} port:{}".format(redis_host,redis_port))
    ...
    try:
        ...
        g_log.info("Starting service {}: port={:d}, PID={:d}".format(args.service,int(service_port), int(pid)))
        try:
