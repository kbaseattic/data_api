...
class AbstractAssemblyAPI(ObjectAPI):
...
class AssemblyAPI(AbstractAssemblyAPI):
    def __init__(self, services, ref):
        ...
        if is_assembly_type:
            ...
            self.proxy = _KBaseGenomes_ContigSet(services, ref)
...
class _KBaseGenomes_ContigSet(AbstractAssemblyAPI):
    def get_assembly_id(self):
        return self.get_data_subset(path_list=["id"])["id"]
    
    def get_genome_annotations(self):
        import biokbase.data_api.annotation.genome_annotation
        return [biokbase.data_api.annotation.genome_annotation.GenomeAnnotationAPI(self.services, self.ref)]
    ...
    def get_contigs(self, contig_id_list=None):
        ...
        for c in matches:
            ...
            if c.has_key("replicon_geometry"):
...
class _Prototype(AbstractAssemblyAPI):
    def get_assembly_id(self):
        ...
        for object_type in referrers:
            if object_type.split('-')[0] in biokbase.data_api.annotation.genome_annotation.TYPES:
                for x in referrers[object_type]:
                    annotations.append(biokbase.data_api.annotation.GenomeAnnotationAPI(self.services, ref=x))
