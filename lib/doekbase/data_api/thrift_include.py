...
class ThriftPP(object):
    ...
    INC_END_MARKER = '#%endinclude'
    INC_END = re.compile('#\s*%endinclude')
    ...
    def _update(self, lines):
        ...
        while i < len(lines):
            ...
            if match:
                ...
                end_pos = self._find_end(lines, i + 1)
                if end_pos > 0:
                    ...
                    result.append(self.INC_END_MARKER)
                    i += 1
            else:
    ...
    def _find_end(self, lines, pos):
        i = pos
        found = -1
        while i < len(lines):
            if self.INC_END.match(lines[i]):
                found = i
                break
            i += 1
        return found
